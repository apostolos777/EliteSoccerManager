...
try {
    ...
    // If a modern many-to-many mapping exists, prefer that for selected teams
    try {
        $r = $db->query("SELECT name FROM sqlite_master WHERE type='table' AND name='player_teams'")->fetch(PDO::FETCH_ASSOC);
        if ($r) {
            $stmt = $db->prepare('SELECT team_id FROM player_teams WHERE player_id = ?');
            $stmt->execute([$player_id]);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            if (!empty($rows)) {
                $teamIds = array_column($rows, 'team_id');
                $player['team_ids'] = implode(',', $teamIds);
            }
        }
    } catch (Exception $e) {
        // ignore if table isn't present
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // No age limit validation (allow any age)
        ...
        if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] === UPLOAD_ERR_OK) {
            ...
            // Delete previous profile image file if it exists (replace behaviour)
            if (!empty($player['profile_image']) && file_exists($player['profile_image'])) {
                @unlink($player['profile_image']);
            }
        ...
        // Document uploads: id_document, passport_document, birth_certificate_document
        $documentUploadDir = 'uploads/players/documents/';
        if (!is_dir($documentUploadDir)) {
            mkdir($documentUploadDir, 0755, true);
        }

        // helper to detect player_documents table
        ...
        $handleDocumentUpload = function($inputName, $docType) use (&$db, $player_id, $documentUploadDir, $hasPlayerDocumentsTable) {
            if (!isset($_FILES[$inputName]) || $_FILES[$inputName]['error'] !== UPLOAD_ERR_OK) return;
            $ext = pathinfo($_FILES[$inputName]['name'], PATHINFO_EXTENSION);
            $dest = $documentUploadDir . 'player_' . $player_id . '_' . $docType . '_' . time() . '.' . $ext;
            if (move_uploaded_file($_FILES[$inputName]['tmp_name'], $dest)) {
                if ($hasPlayerDocumentsTable) {
                    try {
                        ...
                        $stmt->execute([$player_id, $docType, basename($dest), $dest, mime_content_type($dest), filesize($dest)]);
                    } catch (Exception $e) {
                        // swallow but log in future
                    }
                }
            }
        };

        // process known document inputs
        $handleDocumentUpload('id_document', 'id');
        $handleDocumentUpload('passport_document', 'passport');
        $handleDocumentUpload('birth_certificate_document', 'birth_certificate');
                ...
                if ($hasPlayerTeams) {
                    ...
                    if (!empty($selectedTeams) && is_array($selectedTeams)) {
                        ...
                        foreach ($selectedTeams as $t) {
                            $insertStmt->execute([$player_id, (int)$t]);
                        }
                    }
                }
        ...
        <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select id="primary_position" name="primary_position">
                            ...
                            foreach ($positionOptions as $k => $label) {
                                $sel = (($player['primary_position'] ?? '') === $k) ? 'selected' : '';
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label for="id_document">Upload ID / License</label>
                        <input type="file" id="id_document" name="id_document" accept="application/pdf,image/*">
                        <small class="text-muted">PDF or image (Max 10MB). Use this to upload a copy of ID or driving licence.</small>
                    </div>
                    <div class="form-group">
                        <label for="passport_document">Upload Passport</label>
                        <input type="file" id="passport_document" name="passport_document" accept="application/pdf,image/*">
                        <small class="text-muted">PDF or image (Max 10MB). Upload passport page showing name and photo.</small>
                    </div>
                    <div class="form-group">
                        <label for="birth_certificate_document">Upload Birth Certificate</label>
                        <input type="file" id="birth_certificate_document" name="birth_certificate_document" accept="application/pdf,image/*">
