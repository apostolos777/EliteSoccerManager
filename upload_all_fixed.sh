#!/bin/bash
# VIVO United Football Manager - Complete FTP Upload Script
# Updated: August 29, 2025
# This script uploads all fixed files to the live server

echo "🚀 Starting VIVO United Complete Upload..."
echo "=========================================="
...
upload_file() {
    ...
    if [ -f "$local_file" ]; then
        ...
        if curl -s -T "$local_file" "ftp://$FTP_USER:$FTP_PASS@$FTP_HOST/$FTP_PATH/$remote_file"; then
...
upload_directory() {
    ...
    if [ -d "$local_dir" ]; then
        echo -e "${BLUE}📁 Processing directory:${NC} $local_dir"

        # Create remote directory first
        curl -s "ftp://$FTP_USER:$FTP_PASS@$FTP_HOST/$FTP_PATH/$remote_dir/" > /dev/null 2>&1

        # Upload all files in directory
        find "$local_dir" -type f -name "*.php" -o -name "*.js" -o -name "*.css" -o -name "*.json" | while read -r file; do
            # Get relative path
            relative_path="${file#$local_dir/}"
            upload_file "$file" "$remote_dir/$relative_path"
        done
    else
        echo -e "${YELLOW}⚠️  Directory not found:${NC} $local_dir"
    fi
}
...
# 10. Include Files
upload_directory "includes" "includes"
