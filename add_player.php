...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Validate date_of_birth and compute age bounds
        if (!empty($insertData['date_of_birth'])) {
            try {
                $dob = new DateTime($insertData['date_of_birth']);
                $now = new DateTime();
                $ageComputed = $now->diff($dob)->y;
                if ($ageComputed < 3 || $ageComputed > 100) {
                    throw new Exception('Date of birth indicates age outside allowed range (3-100).');
                }
                // overwrite computed age if allowed
                if (in_array('age', $schemaColumns)) {
                    $insertData['age'] = $ageComputed;
                }
            } catch (Exception $e) {
    ...
    // No age limit validation (allow any age)
        ...
        // Additional validation: jersey uniqueness (if team assigned)
        if (!empty($insertData['jersey_number']) && !empty($insertData['team_id']) && in_array('jersey_number', $schemaColumns)) {
            $jnum = (int)$insertData['jersey_number'];
            if ($jnum < 0 || $jnum > 99) throw new Exception('Jersey number must be between 0 and 99');
            $stmt = $db->prepare('SELECT id FROM players WHERE jersey_number = ? AND team_id = ? LIMIT 1');
            $stmt->execute([$jnum, (int)$insertData['team_id']]);
            if ($e = $stmt->fetch(PDO::FETCH_ASSOC)) {
                throw new Exception('Jersey number already taken for that team');
            }
        }

        // Validate contact number (basic)
        if (!empty($insertData['contact_number'])) {
            $cn = preg_replace('/[^0-9+]/', '', $insertData['contact_number']);
            if (strlen($cn) < 6) throw new Exception('Contact number appears too short');
            $insertData['contact_number'] = $cn;
        }

        // Validate social URLs - sanitize invalid ones to null
        $socialKeys = ['facebook_url','instagram_url','twitter_url','tiktok_url','youtube_url','linkedin_url'];
        foreach ($socialKeys as $sk) {
            if (!empty($insertData[$sk]) && !filter_var($insertData[$sk], FILTER_VALIDATE_URL)) {
                $insertData[$sk] = null;
            }
        }

        // Duplicate detection: check for existing player with same name and DOB
        $duplicateQuery = null;
        if ($hasFirstLastName && !$hasNameSurname) {
            $duplicateQuery = $db->prepare('SELECT id FROM players WHERE first_name = ? AND last_name = ?' . (in_array('date_of_birth', $schemaColumns) ? ' AND date_of_birth = ?' : '') . ' LIMIT 1');
            $params = [$insertData['first_name'] ?? '', $insertData['last_name'] ?? ''];
            if (in_array('date_of_birth', $schemaColumns)) $params[] = $insertData['date_of_birth'] ?? '';
        } else {
            $duplicateQuery = $db->prepare('SELECT id FROM players WHERE name = ? AND surname = ?' . (in_array('date_of_birth', $schemaColumns) ? ' AND date_of_birth = ?' : '') . ' LIMIT 1');
            $params = [$insertData['name'] ?? '', $insertData['surname'] ?? ''];
            if (in_array('date_of_birth', $schemaColumns)) $params[] = $insertData['date_of_birth'] ?? '';
        }
        if ($duplicateQuery) {
            $duplicateQuery->execute($params);
            if ($duplicateQuery->fetch(PDO::FETCH_ASSOC)) {
                throw new Exception('A player with the same name and date of birth already exists.');
            }
        }
