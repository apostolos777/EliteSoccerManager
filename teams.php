...
<body>
    ...
    <div class="content-main">
                ...
                <div class="team-card team-item" data-team-name="<?= strtolower($team['name'] ?: '') ?>">
                    ...
                    <div class="tc-body">
                        ...
                        <div class="tc-actions">
                            ...
                            <a href="#" class="btn btn-sm btn-outline-primary" onclick="syncTeamRoster(<?= $team['id'] ?>, this)"><i class="fas fa-sync"></i> Sync roster</a>
                            <?php endif; ?>
                            <?php if (function_exists('isAdmin') && isAdmin()): ?>
        ...
        <script>
            ...
            function syncTeamRoster(teamId, el) {
                if (!confirm('Synchronize roster for this team from players assignments?')) return;
                el.disabled = true;
                fetch('sync_team_roster.php', {
                    method: 'POST',
                ...
                .then(data => {
                    el.disabled = false;
                    if (data.success) {
                        alert('Roster synced. Inserted: ' + (data.inserted || 0) + ', Updated: ' + (data.updated || 0));
                        setTimeout(() => location.reload(), 700);
                    } else {
                        alert('Error syncing roster: ' + (data.message || 'unknown'));
                    }
                })
                .catch(err => { el.disabled = false; console.error(err); alert('Sync failed'); });
            }
