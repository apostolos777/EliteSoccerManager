...
function parseAttendanceCSV($filePath, $db) {
    ...
    while (($row = fgetcsv($handle)) !== false) {
        ...
        // Map columns
        foreach (['player_id', 'event_id', 'status', 'notes'] as $field) {
            if (isset($headerMap[$field])) {
                $record[$field] = $row[$headerMap[$field]] ?? '';
            }
        }
...
function parseAttendanceJSON($filePath, $db) {
    ...
    foreach ($data as $record) {
        ...
        // Validate player exists
        $playerStmt = $db->prepare("SELECT id FROM players WHERE id = ?");
        $playerStmt->execute([(int)($normalized['player_id'] ?? 0)]);
        if (!$playerStmt->fetch()) {
            $normalized['error'] = "Player not found";
        }
        
        // Validate event exists
        $eventStmt = $db->prepare("SELECT id FROM events WHERE id = ?");
        $eventStmt->execute([(int)($normalized['event_id'] ?? 0)]);
        if (!$eventStmt->fetch()) {
            $normalized['error'] = "Event not found";
        }
