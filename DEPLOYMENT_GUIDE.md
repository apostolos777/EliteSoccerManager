...
### Incremental / Changed-file Deploys
For quick uploads of only changed files, use the `deploy_changed.sh` helper in the project root. It detects changed files from Git and uploads them via FTP.

Usage:
```bash
# dry-run lists changed files
FTP_HOST=ftp.example.com FTP_USER=me FTP_PASS=secret FTP_PATH=public_html/vivoapp ./deploy_changed.sh --dry-run

# upload changed files comparing to origin/main
FTP_HOST=ftp.example.com FTP_USER=me FTP_PASS=secret FTP_PATH=public_html/vivoapp ./deploy_changed.sh --range origin/main...HEAD
```

Notes:
- The script uses `curl --ftp-create-dirs` and respects the repo path structure when uploading files.
- Make sure your `.gitignore` and `upload` intentions are correct before doing an incremental deploy.
