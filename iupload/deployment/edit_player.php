...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($hasFirstLastName && !$hasNameSurname) {
            ...
            $updateData['last_name'] = $_POST['surname'] ?? '';
        } else {
            // Local schema: use name/surname
            $updateData['name'] = $_POST['name'] ?? '';
            $updateData['surname'] = $_POST['surname'] ?? '';
        }
        ...
        $nameValid = false;
        if ($hasFirstLastName && !$hasNameSurname) {
            $nameValid = !empty($updateData['first_name']) && !empty($updateData['last_name']);
        } else {
            $nameValid = !empty($updateData['name']) && !empty($updateData['surname']);
        }
